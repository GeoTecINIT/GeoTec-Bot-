<?php
  ...
  function parse_set_tag(&$convoArr, $element, $parentName, $level)
  {
    ...
    if ($rowCount == 0)
    {
      $sql = "insert into `$dbn`.`client_properties` (`id`, `user_id`, `bot_id`, `name`, `value`, `timestamp`)
      values (NULL, $user_id, $bot_id, '$var_name', '$var_value', CURRENT_TIMESTAMP);";
      runDebug(__FILE__, __FUNCTION__, __LINE__, "No value found for $var_name. Inserting $var_value into the table.", 4);
    }
    else
    {
      $sql = "update `$dbn`.`client_properties` set `value` = '$var_value', `timestamp` = CURRENT_TIMESTAMP where `user_id` = $user_id and `bot_id` = $bot_id and `name` = '$var_name';";
